...
namespace ImageProcessing02.Athorithm
{
    class RAMTool
    {
        ...
        */
        ...
        //用二值数组新建一个1位二值图像
        private static Bitmap two(byte[] rawValues, int width, int height)
        {
            //新建一个1位二值位图，并锁定内存区域操作
            Bitmap bitmap = new Bitmap(width, height, PixelFormat.Format1bppIndexed);
            BitmapData bitmapData = bitmap.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.WriteOnly, PixelFormat.Format1bppIndexed);

            //计算图像参数
            int offset = bitmapData.Stride - bitmapData.Width / 8; //计算每行未用空间字节数
            IntPtr ptr = bitmapData.Scan0; //获取首地址
            int scanBytes = bitmapData.Stride * bitmapData.Height; //图像字节数 = 扫描字节书 * 高度
            byte[] grayValues = new byte[scanBytes]; //为图像数据分配内存
            ...
            for(int i = 0; i < height; i++)
            {
                for(int j = 0; j < bitmapData.Width / 8; j++)
                {
            ...
            //内存解锁
            Marshal.Copy(grayValues, 0, ptr, scanBytes);
            bitmap.UnlockBits(bitmapData); // 结束内存区域

            //修改生成位图的索引表
            ColorPalette palette;
            //获取一个Format8bppIndexd格式图像的Palette对象
